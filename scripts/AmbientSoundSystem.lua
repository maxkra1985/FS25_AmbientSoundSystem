...
function AmbientSoundSystem.new()
	local self = setmetatable({}, AmbientSoundSystem_mt)
	self.soundFiles = {}
	self.sounds = {}
	self.scheduler = nil
	self.activeSounds = {}
...
function AmbientSoundSystem:initialize()
	...
	self.soundFiles, self.sounds = AmbientSoundXML.load(self.xmlFilename)
	self:prepareSounds()
	self.scheduler = AmbientSoundScheduler.new(self.sounds)
	self.scheduler:reset()
	g_ambientSoundSystem = self
	AmbientSoundUtil.info("Ambient Sound System готов")
	return true
end
...
function AmbientSoundSystem:prepareSounds()
	for _, sound in ipairs(self.sounds) do
		sound.fileCache = self.soundFiles
		AmbientSoundUtil.debug("Подготовлен звук ID=%d", sound.id)
	end
end
...
function AmbientSoundSystem:updateScheduler(dt)
	local readySounds = self.scheduler:update(dt)
	for _, config in ipairs(readySounds) do
		self:createSound(config)
	end
end
...
function AmbientSoundSystem:createSound(config)
	local position = self:getSoundPosition(config)
	if position == nil then
		AmbientSoundUtil.warning("Не удалось определить позицию звука ID=%d", config.id)
		return
	end
	local sound = AmbientSound.new(config, position)
	if sound:load() then
		------------------------------------------------------------------------------
		-- Запуск звука
		------------------------------------------------------------------------------
		if config.type == "global" then
			if AmbientSoundUtil.isServer() then
				AmbientSoundPlayEvent.sendEvent(config, position)
			end
		else
			sound:play()
		...
		table.insert(self.activeSounds, sound)
		AmbientSoundUtil.debug("Создан активный звук ID=%d",config.id)
	end
...
function AmbientSoundSystem:getSoundPosition(config)
	...
	if config.type == "local" then
		return self:getPlayerSoundPosition(config)
	end

	--------------------------------------------------------------------------
	-- Статическая глобальная точка
	--------------------------------------------------------------------------
	if config.mode == "static" and config.translation ~= nil then
		local position = {
		...
		if config.randomRadius > 0 then
			return AmbientSoundUtil.randomPointInRadius(position.x, position.y, position.z, config.randomRadius)
		end
		return position
	end
...
------------------------------------------------------------------------------
function AmbientSoundSystem:getPlayerSoundPosition(config)
	local player = self:getRandomPlayer()
	if player == nil then
		return nil
	end
	local x, y, z = getWorldTranslation(player.rootNode)
	local radius = config.distancePlayer
	if radius <= 0 then
		radius = 5
	end
	return AmbientSoundUtil.randomPointInRadius(x, y, z, radius)
end
...
function AmbientSoundSystem:getRandomPlayer()
	...
	return AmbientSoundUtil.randomElement(players)
end
...
function AmbientSoundSystem:updateActiveSounds(dt)
	for index = #self.activeSounds, 1, -1 do
		local sound = self.activeSounds[index]
		sound:update(dt)
		if not sound:isActive() then
			if sound.config.type == "global" and AmbientSoundUtil.isServer() then
				AmbientSoundStopEvent.sendEvent(sound.config, index)
			end
			sound:delete()
...
function AmbientSoundSystem:delete()
	...
	AmbientSoundUtil.info("Ambient Sound System удалён")
end